...
[Pulsar admin APIs](admin-get-started.md)| {@inject: rest:REST API:/}, which lists all parameters, responses, samples, and more.|See the `/admin/v3/functions` endpoint
[Pulsar admin APIs](admin-get-started.md)|[Java admin API](@pulsar:javadoc:admin@/), which lists all classes, methods, descriptions, and more.|See the `functions` method of the `PulsarAdmin` object
