...
In Pulsar, the highest level of message delivery guarantee is using an [idempotent producer](concepts-messaging-legacy.md#producer-idempotency) with the exactly once semantic at one single partition, that is, each message is persisted exactly once without data loss and duplication. However, there are some limitations in this solution:
