...
The Pulsar admin interface enables you to create and manage Pulsar Functions through CLI. For the latest and complete information, including commands, flags, and descriptions, refer to [Pulsar admin CLI](/reference/#/@pulsar:version_reference@/pulsar-admin/).
...
| maxMessageRetries    | Int      |	`--max-message-retries` | The number of retries to process a message before giving up.<br />**Note:** Supported by Java and Python functions. For Python functions, the value must be at least `1`, because the Python client requires a redelivery count of `1` or greater, and the worker needs `pulsar-client` 3.3.0 or later (see [Package Python functions](functions-package-python.md)). Go functions do not support this field. |
| deadLetterTopic      | String   | `--dead-letter-topic`   | The topic used for storing messages that are not processed successfully.<br />**Note:** Supported by Java and Python functions; requires `maxMessageRetries` to be set. For Python functions, the worker needs `pulsar-client` 3.3.0 or later (see [Package Python functions](functions-package-python.md)). If the topic is left empty, the client derives `<topic>-<subscription>-DLQ`. Go functions do not support this field. |
...
| maxPendingAsyncRequests | Int    | N/A                        | The max number of pending async requests per instance to avoid a large number of concurrent requests. |
