...
| maxMessageRetries    | Int      |	`--max-message-retries` | The number of retries to process a message before giving up.<br />**Note:** Supported by Java and Python functions. For Python functions, the value must be at least `1`, because the Python client requires a redelivery count of `1` or greater. Go functions do not support this field. |
| deadLetterTopic      | String   | `--dead-letter-topic`   | The topic used for storing messages that are not processed successfully.<br />**Note:** Supported by Java and Python functions; requires `maxMessageRetries` to be set. Go functions do not support this field. |
