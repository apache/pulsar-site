...
DB2 connector does not include DB2 JDBC driver and you need to package it with the connector.
Major reasons for not including the drivers are the variety of versions and DB2 licensing. It is recommended to use the driver provided with your DB2 DB installation, or you can [download](https://www.ibm.com/support/pages/db2-jdbc-driver-versions-and-downloads) one.
Integration tests have an [example](https://github.com/apache/pulsar/pull/19821/files#diff-81eb2a00c15e691885f29a1a24862bcbbed7d264df34471ae01b068888406fddR133-R143) of packaging the driver into the connector NAR file. An example is also provided below. 

### Configuration

Debezium for DB2 [requires](https://debezium.io/documentation/reference/2.2/connectors/db2.html) the use of the abstract syntax notation (ASN) libraries, which are included with DB2 for Linux. To use the ASN libraries, you must have a license for IBM InfoSphere Data Replication (IIDR). You do not have to install IIDR to use the ASN libraries.
The steps outlined in the [documentation](https://debezium.io/documentation/reference/2.2/connectors/db2.html) and used in the [integration test](https://github.com/devinbost/pulsar/blob/4ea52168bc0e8c6531ae8e276d6373483f07c81e/tests/integration/src/test/java/org/apache/pulsar/tests/integration/io/sources/debezium/DebeziumDB2DbSourceTester.java) may or may not work for the version and edition of Oracle DB you are using.
Please refer to the [documentation for IBM DB2 Data Replication](https://www.ibm.com/products/data-replication) as needed.

The Nar file can be built for the connector like this:

```bash
PULSAR_VERSION=2.11.0
cd /
curl -O https://archive.apache.org/dist/pulsar/pulsar-${PULSAR_VERSION}/apache-pulsar-${PULSAR_VERSION}-bin.tar.gz
tar -zxvf ./apache-pulsar-${PULSAR_VERSION}-bin.tar.gz
# Build the DB2 Nar package
mkdir -p /apache-pulsar-${PULSAR_VERSION}/META-INF/bundled-dependencies
cd /apache-pulsar-${PULSAR_VERSION}/META-INF/bundled-dependencies && curl -sSLO https://search.maven.org/remotecontent?filepath=com/ibm/db2/jcc/11.5.8.0/jcc-11.5.8.0.jar
cd /apache-pulsar-${PULSAR_VERSION}/META-INF/bundled-dependencies && curl -sSLO https://search.maven.org/remotecontent?filepath=com/ibm/db2/jcc/db2jcc/db2jcc4/db2jcc-db2jcc4.jar
mkdir -p /apache-pulsar-${PULSAR_VERSION}/connectors

# If building from source, then you must copy the Nar into /apache-pulsar-${PULSAR_VERSION}/connectors before adding the required libraries

cp /proj/pulsar/pulsar-io/debezium/db2/target/pulsar-io-debezium* /apache-pulsar-${PULSAR_VERSION}/connectors

cd /apache-pulsar-${PULSAR_VERSION} && jar uf connectors/pulsar-io-debezium-db2-*.nar META-INF/bundled-dependencies/jcc-11.5.8.0.jar META-INF/bundled-dependencies/db2jcc-db2jcc4.jar
```

Before starting the connector, be sure to enable retention on the namespace.
```bash
cd /apache-pulsar-${PULSAR_VERSION} && bin/pulsar-admin namespaces set-retention public/default -t -1 -s 100M
```
Then, connect to your DB2 instance, start the CDC agent, create your desired table, and add it to CDC:
```bash
alias db2=/opt/ibm/db2/V11.5/bin/db2
db2 connect to mydb2 user 'db2inst1' using 'admin'
db2 bind db2schema.bnd blocking all grant public sqlerror continue
db2 
VALUES ASNCDC.ASNCDCSERVICES('start','asncdc');
CREATE TABLE DB2INST1.STORES(store_id INT GENERATED BY DEFAULT AS IDENTITY NOT NULL,store_name VARCHAR(150) NOT NULL,state_id INT NOT NULL,zip_code VARCHAR(6),PRIMARY KEY (store_id));
INSERT INTO DB2INST1.STORES(store_name, state_id, zip_code) VALUES ('mystore', 10, '11111');
call ASNCDC.ADDTABLE('DB2INST1','STORES')
```

Similarly to other connectors, you can use JSON or YAML to configure the connector.
In this example, our connection information is mapped to a DB2 instance started this way:
```bash
sudo docker run --privileged=true --user 0 -e "LICENSE=accept" -e "DB2INSTANCE=db2inst1" -e "DB2INST1_PASSWORD=admin" -e "DBNAME=mydb2" -e "ENABLE_ORACLE_COMPATIBILITY=false" -e "UPDATEAVAIL=NO" -e "IS_OSXFS=false" -e "REPODB=false" -e "PERSISTENT_HOME=true" devingbost/debezium-db2:0.0.1 --name db2
bash
...
"archive": "connectors/pulsar-io-debezium-db2-3.0.0-SNAPSHOT.nar",
"parallelism": 1,
"configs": {
   "database.hostname": "localhost",
   "database.port": "50000",
   ...
   "topic.prefix": "stores",
   "table.exclude.list": "DB2INST1.EXCLUDEDSTORES",
   "database.history": "org.apache.pulsar.io.debezium.PulsarDatabaseHistory",
...
```
Using YAML as an example, you can create a `debezium-db2-source-config.yaml` file like:
* YAML 
...
For the full list of configuration properties supported by Debezium, see [Debezium Connector for DB2](https://debezium.io/documentation/reference/stable/connectors/db2.html#db2-connector-properties).

You can start the connector like this:
```bash
cd /apache-pulsar-${PULSAR_VERSION}; bin/pulsar-admin source localrun  --sourceConfigFile /apache-pulsar-${PULSAR_VERSION}/debezium-db2-source-config.yaml --archive /apache-pulsar-${PULSAR_VERSION}/connectors/${NAR_FILE}
```

In this example, you can consume from the topic using the CLI in this way:
```bash
bin/pulsar-client consume -s test -p Earliest -n 0 -m NonDurable -st auto_consume persistent://public/default/db2inst1.DB2INST1.STORES
```
