---
id: concepts-messaging-legacy
title: Messaging (classic topics)
sidebar_label: "Messaging (classic)"
description: The classic Pulsar messaging model — partitioned and non-partitioned topics with the Exclusive, Failover, Shared, and Key_Shared subscription types and the v4 client API.
---
...
This page describes the **classic topic model** and the v4 client API: partitioned and non-partitioned topics, the four subscription types (Exclusive, Failover, Shared, Key_Shared), and mod-N partition routing. It remains fully supported.

For the current model based on [scalable topics](concepts-scalable-topics.md) and the V5 client API, see [Messaging](concepts-messaging.md). New applications should start there.
...
| `CustomPartition`     | Use custom message router implementation that will be called to determine the partition for a particular message. User can create a custom routing mode by using the [Java client](/docs/client-libraries/java) and implementing the [MessageRouter](@pulsar:javadoc:client@/org/apache/pulsar/client/api/MessageRouter) interface.                                                                                                                                  |
...
[Hashing Scheme](@pulsar:javadoc:client@/org/apache/pulsar/client/api/HashingScheme) is an enum that represents sets of standard hashing functions available when choosing the partition to use for a particular message.
...
By default, non-persistent topics are enabled on Pulsar brokers. You can disable them in the broker's [configuration](reference-configuration.md#broker-enableNonPersistentTopics). You can manage non-persistent topics using the `pulsar-admin topics` command. For more information, see [`pulsar-admin`](/reference/#/@pulsar:version_reference@/pulsar-admin/).
...
* You cannot create any system topics. To list system topics, you can add the option `--include-system-topic` when you get the topic list by using [Pulsar admin API](/reference/#/@pulsar:version_reference@/pulsar-admin/).
