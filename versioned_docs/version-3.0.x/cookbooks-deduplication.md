...
Though message deduplication is disabled by default at the broker level, you can enable message deduplication for a specific namespace or topic using the [`pulsar-admin namespaces set-deduplication`](/reference/#/@pulsar:version_reference@/pulsar-admin/namespaces?id=set-deduplication) or the [`pulsar-admin topics set-deduplication`](/reference/#/@pulsar:version_reference@/pulsar-admin/topics?id=set-deduplication) command. You can use the `--enable`/`-e` flag and specify the namespace/topic.
...
Even if you enable message deduplication at the broker level, you can disable message deduplication for a specific namespace or topic using the [`pulsar-admin namespace set-deduplication`](/reference/#/@pulsar:version_reference@/pulsar-admin/namespaces?id=set-deduplication) or the [`pulsar-admin topics set-deduplication`](/reference/#/@pulsar:version_reference@/pulsar-admin/topics?id=set-deduplication) command. Use the `--disable`/`-d` flag and specify the namespace/topic.
...
To ensure the guarantee order on a [Java producer](pathname:///docs/client-libraries/java-use#create-a-producer) sending to a topic with message deduplication enabled, set the producer name using the `producerName` setter, and set the timeout to `0` using the `sendTimeout` setter.
...
Not to break the guarantee order on a [Python producer](pathname:///docs/client-libraries/python-use#create-a-producer) sending to a topic with message deduplication active, set the producer name using `producer_name`, and set the timeout to `0` using `send_timeout_millis`.
...
Not to break the guarantee order on a [C++ producer](pathname:///docs/client-libraries/cpp-use#create-a-producer) sending to a topic with message deduplication active, set the producer name using `producer_name`, and set the timeout to `0` using `send_timeout_millis`.
