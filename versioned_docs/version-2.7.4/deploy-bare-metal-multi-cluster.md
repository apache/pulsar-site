...
## System requirement
Pulsar is currently available for **MacOS** and **Linux**. See [Run Pulsar In Docker](getting-started-docker.md) if you want to run Pulsar on **Windows**. In order to use Pulsar, you need to install Java 8 from [Oracle download center](http://www.oracle.com/).
...
[Clients](concepts-clients.md) connecting to Pulsar brokers need to be able to communicate with an entire Pulsar instance using a single URL. Pulsar provides a built-in service discovery mechanism that you can set up using the instructions [immediately below](#service-discovery-setup).
