...
To ensure the guarantee order on a [Java producer](pathname:///docs/client-libraries/java#producer) sending to a topic with message deduplication enabled, set the producer name using the `producerName` setter, and set the timeout to `0` using the `sendTimeout` setter.
...
Not to break the guarantee order on a [Python producer](pathname:///docs/client-libraries/python) sending to a topic with message deduplication active, set the producer name using `producer_name`, and set the timeout to `0` using `send_timeout_millis`.
...
Not to break the guarantee order on a [C++ producer](pathname:///docs/client-libraries/cpp#create-a-producer) sending to a topic with message deduplication active, set the producer name using `producer_name`, and set the timeout to `0` using `send_timeout_millis`.
