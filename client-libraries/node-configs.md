...
| `authentication` | Configure the authentication provider. (default: no authentication). See [mTLS authentication](pathname:///docs/security-tls-authentication) for more info. | |
| `operationTimeoutSeconds` | The timeout for Node.js client operations (creating producers, subscribing to and unsubscribing from [topics](pathname:///docs/reference-terminology#topic)). Retries occur until this threshold is reached, at which point the operation fails. | 30 |
...
| Parameter | Description | Default |
| :-------- | :---------- | :------ |
| `topic` | The Pulsar [topic](pathname:///docs/reference-terminology#topic) to which the producer publishes messages. The topic format is `<topic-name>` or `<tenant-name>/<namespace-name>/<topic-name>`. For example, `sample/ns1/my-topic`. | |
...
| `sendTimeoutMs` | When publishing a message to a topic, the producer waits for an acknowledgment from the responsible Pulsar [broker](pathname:///docs/reference-terminology#broker). If a message is not acknowledged within the threshold set by this parameter, an error is thrown. If you set `sendTimeoutMs` to -1, the timeout is set to infinity (and thus removed). Removing the send timeout is recommended when using Pulsar's [message de-duplication](pathname:///docs/cookbooks-deduplication) feature. | 30000 |
...
| `maxPendingMessages` | The maximum size of the queue holding pending messages (i.e. messages waiting to receive an acknowledgment from the [broker](pathname:///docs/reference-terminology#broker)). By default, when the queue is full all calls to the `send` method fails *unless* `blockIfQueueFull` is set to `true`. | 1000 |
...
| `consumerName` | The name of consumer. Currently(v2.4.1), [failover](pathname:///docs/concepts-messaging#failover) mode use consumer name in ordering. | |
...
| Parameter | Description | Default |
| :-------- | :---------- | :------ |
| `topic` | The Pulsar [topic](pathname:///docs/reference-terminology#topic) on which the reader establishes a subscription and listen for messages. | |
