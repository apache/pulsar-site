...
You can configure Node.js producers using a [producer configuration](node-configs.md#producer-configuration) object.
...
You can configure Node.js consumers using a [consumer configuration](node-configs.md#consumer-configuration) object.
...
Pulsar readers are different from consumers because with readers you need to explicitly specify which message in the stream you want to begin with (consumers, on the other hand, automatically begin with the most recently unacked message). You can configure Node.js readers using a [reader configuration](node-configs.md#reader-configuration) object.
